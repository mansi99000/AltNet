...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        if action_space.dtype == np.dtype(np.float64):
            action_space.dtype = np.dtype(np.float32)
        ...
        self.index = np.zeros(self.buffer_size, dtype=np.float32)
    ...
    def add(
        ...
        if action_index is not None:
            if type(action_index) is not float and type(action_index) is not int:
                action_index = action_index.cpu()
            self.index[self.pos] = np.array(action_index).copy()
