...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _sample_action(
        ...
        action_noise: Optional[ActionNoise] = None,
        n_envs: int = 1,
    ) -> Tuple[np.ndarray, np.ndarray, Union[int, float, None]]:
        """
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            unscaled_action, _, action_index = self.predict(self._last_obs, deterministic=False)
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            ...
            buffer_action = np.float32(scaled_action)
            action = self.policy.unscale_action(scaled_action)
        else:
            # Discrete case, no need to normalize or clip
            buffer_action = unscaled_action
            action = buffer_action
        return action, buffer_action, action_index
    ...
    def _store_transition(
        ...
        action_index: Union[int, float, None] = None,
    ) -> None:
        """
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            # Select action randomly or according to policy
            actions, buffer_actions, action_index = self._sample_action(learning_starts, action_noise, env.num_envs)
            ...
            self._store_transition(replay_buffer, buffer_actions, new_obs, rewards, dones, infos, action_index)
