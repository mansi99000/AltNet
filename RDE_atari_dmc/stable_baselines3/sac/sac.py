...
class SAC(OffPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        for i in range(self.num_agent):
            self.batch_norm_stats.append(get_parameters_by_name(self.critic[i], ["running_"]))
            self.batch_norm_stats_target.append(get_parameters_by_name(self.critic_target[i], ["running_"]))
        ...
        if isinstance(self.ent_coef, str) and self.ent_coef.startswith("auto"):
            ...
            for i in range(self.num_agent):
                self.log_ent_coef.append(th.log(th.ones(1, device=self.device) * init_value).requires_grad_(True))
                self.ent_coef_optimizer.append(th.optim.Adam([self.log_ent_coef[i]], lr=self.lr_schedule(1)))
        else:
    ...
    def _create_aliases(self) -> None:
        ...
        self.critic = []
        self.critic_target = []
        for i in range(self.policy.num_agent):
            self.actor.append(getattr(self.policy, f"actor{i}"))
            self.critic.append(getattr(self.policy, f"critic{i}"))
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            for i in range(self.num_agent):
                ...
                ent_coef = th.exp(self.log_ent_coef[i].detach())
                ent_coef_loss = -(self.log_ent_coef[i] * (log_prob + self.target_entropy).detach()).mean()
                ent_coef_losses.append(ent_coef_loss.item())

                ent_coefs.append(ent_coef.item())

                self.ent_coef_optimizer[i].zero_grad()
                ent_coef_loss.backward()
                self.ent_coef_optimizer[i].step()

                with th.no_grad():
                    next_actions, next_log_prob = self.actor[i].action_log_prob(replay_data.next_observations)
                    next_q_values = th.cat(self.critic_target[i](replay_data.next_observations, next_actions),
                                           dim=1)
                    next_q_values, _ = th.min(next_q_values, dim=1, keepdim=True)
                    next_q_values = next_q_values - ent_coef * next_log_prob.reshape(-1, 1)
                    target_q_values = replay_data.rewards + (1 - replay_data.dones) * self.gamma * next_q_values

                current_q_values = self.critic[i](replay_data.observations, replay_data.actions)

                critic_loss = 0.5 * sum(F.mse_loss(current_q, target_q_values) for current_q in current_q_values)
                critic_losses.append(critic_loss.item())

                self.critic[i].optimizer.zero_grad()
                critic_loss.backward()
                self.critic[i].optimizer.step()

                q_values_pi = th.cat(self.critic[i](replay_data.observations, actions_pi), dim=1)
                min_qf_pi, _ = th.min(q_values_pi, dim=1, keepdim=True)

                actor_loss = (ent_coef * log_prob - min_qf_pi).mean()
                actor_losses.append(actor_loss.item())

                self.actor[i].optimizer.zero_grad()
                actor_loss.backward()
                self.actor[i].optimizer.step()
            ...
            ent_coefs_losses.append(ent_coef_losses)
            ent_coefss.append(ent_coefs)
